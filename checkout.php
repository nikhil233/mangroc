...
if(isset($_POST['place_order'])){
	...
	if($is_error==''){
		...
		$sql="insert into order_master(user_id,name,email,mobile,address,zipcode,total_price,order_status,payment_status,added_on,coupon_code,final_price,payment_type) values('".$_SESSION['USER_ID']."','$checkout_name','$checkout_email','$checkout_mobile','$checkout_address','$checkout_zip','$totalPrice','1','pending','$added_on','$coupon_code','$final_price','$payment_type')";
		mysqli_query($con,$sql);
		$insert_id=mysqli_insert_id($con);
		$_SESSION['ORDER_ID']=$insert_id;
		foreach($cartArr as $key=>$val){
			mysqli_query($con,"insert into order_detail(order_id,product_details_id,price,qty) values('$insert_id','$key','".$val['price']."','".$val['qty']."')");
		}
		emptyCart();
		...
		if($payment_type=='wallet'){
			manageWallet($_SESSION['USER_ID'],$final_price,'out','Order Id-'.$insert_id);
		...
		if($payment_type=='paytm'){
			$paytm_oid=$insert_id.'_'.$_SESSION['USER_ID'];
                                                    ...
                                                    <div class="checkout-login">
                                                        ...
                                                        <form method="post" id="frmLogin">
                                                         ...
                                                         <div class="checkout-login-btn ">
                                                            <button type="submit" id="login_submit" class="my_btn site-btn">Login</button>
									...
									<?php foreach($cartArr as $key=>$list){ ?>
										...
										<div class="row">
											<div class="col-md-6 col-6">
												...
												<a href="#"><img alt="" src="<?php echo SITE_PRODUCT_IMAGE.$list['image']?>"></a>
												</div>
											</div>
											<div class="col-md-6 col-6">
