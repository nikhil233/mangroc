...
function getUserCart(){
	...
	$arr=array();
	$id=$_SESSION['USER_ID'];
	$res=mysqli_query($con,"select * from product_cart where user_id='$id'");
	while($row=mysqli_fetch_assoc($res)){
		$arr[]=$row;
	}
	return $arr;
...
function manageUserCart($uid,$qty,$attr){
	...
	if(mysqli_num_rows($res)>0){
		$row=mysqli_fetch_assoc($res);
		$cid=$row['id'];
		mysqli_query($con,"update product_cart set qty='$qty' where id='$cid'");
	}else{
...
/*function getProductCartStatus(){
	global $con;
	$cartArr=array();
	$ProductDetailsID=array();
	if(isset($_SESSION['USER_ID'])){
		$getUserCart=getUserCart();
		$cartArr=array();
		foreach($getUserCart as $list){
			$ProductDetailsID[]=$list['product_detail_id'];
		}
	}else{
		if(isset($_SESSION['cart']) && count($_SESSION['cart'])>0){
			foreach($_SESSION['cart'] as $key=>$val){
				$ProductDetailsID[]=$key;
			}
		}
	}
	
	foreach($ProductDetailsID as $id){
    echo "$id";
		$res=mysqli_query($con,"select product.status as product_status from product where product.id='$id' ");
    $row=mysqli_fetch_assoc($res);
   
		if($row['product_status']==0){
			$id=$row['id'];
			$res=mysqli_query($con,"select id from product where id='$id'");
...
function getUserFullCart($attr_id=''){
	...
	if(isset($_SESSION['USER_ID'])){
		...
		$cartArr=array();
		foreach($getUserCart as $list){
			$cartArr[$list['product_detail_id']]['qty']=$list['qty'];
			$getProductDetail=getProductDetailById($list['product_detail_id']);
			
			$cartArr[$list['product_detail_id']]['price']=$getProductDetail['price'];
			$cartArr[$list['product_detail_id']]['name']=$getProductDetail['name'];
      $cartArr[$list['product_detail_id']]['image']=$getProductDetail['image'];
      $cartArr[$list['product_detail_id']]['attr']=$getProductDetail['attribute'];
		}
	}else{
		if(isset($_SESSION['cart']) && count($_SESSION['cart'])>0){
			...
			foreach($_SESSION['cart'] as $key=>$val){
				...
				$cartArr[$key]['price']=$getProductDetail['price'];
				$cartArr[$key]['name']=$getProductDetail['name'];
        $cartArr[$key]['image']=$getProductDetail['image'];
        $cartArr[$key]['attr']=$getProductDetail['attribute'];
			}
		}
	}
	if($attr_id!=''){
    return $cartArr[$attr_id];
  }
  else{
		return $cartArr;
	}
...
function getProductDetailById($id){
	global $con;
	$res=mysqli_query($con,"select product.name,product.image,product_details.price,product_details.attribute from product_details,product where product_details.id='$id' and product.id=product_details.product_id ");
	$row=mysqli_fetch_assoc($res);
	return $row;
}
...
function removeProductFromCartByid($id){
	...
	if(isset($_SESSION['USER_ID'])){
		global $con;
		$res=mysqli_query($con,"delete from product_cart where product_detail_id='$id' and user_id=".$_SESSION['USER_ID']);
	}else{
...
function getOrderDetails($oid){
	global $con;
	$sql="select order_detail.price,order_detail.qty,product_details.attribute,product.name,order_detail.product_details_id
	from order_detail,product_details,product
	WHERE
	order_detail.order_id=$oid AND
	order_detail.product_details_id=product_details.id AND
                            ...
                            <td colspan="2">
                              <table class="purchase_content" width="100%" cellpadding="0" cellspacing="0">
                                ...
                                foreach($getOrderDetails as $list){
									...
									$html.='<tr>
									  <td width="40%" class="purchase_item"><span class="f-fallback">'.$list['name'].'</span></td>
